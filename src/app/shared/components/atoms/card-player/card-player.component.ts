...
export class CardPlayerComponent {
  ...
  getStyle(hasCard: boolean, isreveal: boolean): string {
    ...
    let spectatorStyle = `w-12 h-12 bg-[#BDBDFF] rounded-full flex items-center justify-center text-primary`;

    // Lógica para el jugador actual
    if (this.viewPlayer === 'player') {
      if (!hasCard) {
        return `${baseStyle} bg-transparent border-white ${isreveal ? 'text-white' : 'text-primary'}`;
      }

      if (hasCard) {
        return `${baseStyle} ${isreveal ? 'bg-transparent text-white' : 'bg-[#BB65FF] shadow-[0px_5px_10px_0px_#6429CD99] text-primary'}`;
      }
    }
